 ...
 const coursesCollection = coursesList.map((course, index) => {
  ...
  return (
    <div data-cy={`course-section-${course.index}`}>
      ...
      <p data-cy={`course-category-${course.index}`} className="capitalized"><span className="bold">Category:</span> {course.category}</p>
      <p data-cy={`course-instructors-${course.index}`}><span className="bold">Instructors:</span> {course.instructors}</p>
      <p data-cy={`course-info-${course.index}`}>{course.info}</p>
      <p data-cy={`course-price-${course.index}`}><span className="bold">Price:</span> {course.price}</p>
    </div>
  );
 });
